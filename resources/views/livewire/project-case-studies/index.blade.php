        ...
        <div class="grid gap-7 xl:grid-cols-[.8fr_1.2fr]">
            <section class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                <div>
                    ...
                    <p class="mt-2 text-sm leading-6 text-slate-600">Basic project data remains managed in the existing Projects module. Select one project here to add professional narrative.</p>
                </div>
                ...
                <div class="mt-5 space-y-3">
                        ...
                        <button type="button" wire:click="edit({{ $project->id }})" class="w-full rounded-2xl border p-4 text-left transition {{ $projectId === $project->id ? 'border-emerald-500 bg-emerald-50' : 'border-slate-200 hover:border-slate-400' }}">
                            <div class="flex items-start justify-between gap-4">
                                <div>
                                    ...
                                    <p class="mt-1 text-xs font-semibold text-slate-500">{{ $project->category ?: 'No category' }} · {{ str_replace('_', ' ', $project->status) }}</p>
                                </div>
                    ...
                    @empty
                        <p class="rounded-2xl border border-dashed border-slate-300 p-6 text-sm text-slate-600">No projects found. Create a project first.</p>
                    @endforelse
                </div>
            ...
            <section class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                @if ($projectId)
                    <div class="flex flex-col gap-3 border-b border-slate-200 pb-6 sm:flex-row sm:items-start sm:justify-between">
                        ...
                        <button type="button" wire:click="clearSelection" class="text-sm font-bold text-slate-500 hover:text-slate-950">Close editor</button>
                    </div>
                    ...
                    <form wire:submit="save" class="mt-7 space-y-6">
                        <div class="grid gap-5 md:grid-cols-2">
                            <label class="block">
                                ...
                                <input wire:model="slug" type="text" class="mt-2 w-full rounded-2xl border-slate-300 px-4 py-3">
                            ...
                            <label class="block">
                                ...
                                <input wire:model="role" type="text" placeholder="Lead Developer, Full-stack Engineer…" class="mt-2 w-full rounded-2xl border-slate-300 px-4 py-3">
                        ...
                        <label class="block">
                            ...
                            <textarea wire:model="summary" rows="3" class="mt-2 w-full rounded-2xl border-slate-300 px-4 py-3" placeholder="One concise paragraph explaining the product and its value."></textarea>
                        ...
                        <label class="block">
                            ...
                            <textarea wire:model="problem" rows="6" class="mt-2 w-full rounded-2xl border-slate-300 px-4 py-3" placeholder="What was broken, inefficient, risky, or missing? Who was affected?"></textarea>
                        ...
                        <label class="block">
                            ...
                            <textarea wire:model="solution" rows="7" class="mt-2 w-full rounded-2xl border-slate-300 px-4 py-3" placeholder="Explain architecture, product decisions, constraints, implementation, and verification."></textarea>
                        ...
                        <label class="block">
                            ...
                            <textarea wire:model="outcome" rows="6" class="mt-2 w-full rounded-2xl border-slate-300 px-4 py-3" placeholder="State verified results, measurable improvements, adoption, or lessons. Do not fabricate metrics."></textarea>
                        ...
                        <div class="grid gap-5 md:grid-cols-[1fr_.35fr]">
                            <label class="block">
                                ...
                                <input wire:model="sourceCodeUrl" type="text" placeholder="github.com/…" class="mt-2 w-full rounded-2xl border-slate-300 px-4 py-3">
                        ...
                        <div class="grid gap-3 rounded-2xl bg-slate-100 p-5 sm:grid-cols-3">
                            <label class="flex items-start gap-3">
                                ...
                                <span><strong class="block text-sm">Public project</strong><small class="text-slate-500">Show on homepage</small></span>
                            </label>
                            <label class="flex items-start gap-3">
                                <input wire:model="isFeatured" type="checkbox" class="mt-1 rounded border-slate-300 text-emerald-700 focus:ring-emerald-600">
                                <span><strong class="block text-sm">Featured</strong><small class="text-slate-500">Prioritize ordering</small></span>
                            </label>
                            <label class="flex items-start gap-3">
                                ...
                                <span><strong class="block text-sm">Publish case study</strong><small class="text-slate-500">Enable detail URL</small></span>
                            </label>
                        </div>
                        ...
                        <div class="flex flex-wrap items-center justify-between gap-4 border-t border-slate-200 pt-6">
                            <p class="text-xs leading-5 text-slate-500">Publication requires completed status plus role, summary, problem, solution, and outcome.</p>
                ...
                @else
                    <div class="grid min-h-[520px] place-items-center text-center">
                        <div class="max-w-md">
                            ...
                            <p class="mt-4 leading-7 text-slate-600">Do not publish an empty case study. The public page is intentionally blocked until the substantive fields are complete.</p>
                        </div>
                    </div>
                @endif
            </section>
