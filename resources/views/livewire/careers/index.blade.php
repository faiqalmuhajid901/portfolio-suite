    ...
    <div class="mx-auto max-w-7xl">
        ...
        <div class="grid gap-7 xl:grid-cols-[.9fr_1.1fr]">
            <section class="rounded-[2rem] border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 p-6 shadow-sm sm:p-8">
                ...
                <p class="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">Write responsibilities and verified achievements. Empty corporate language does not strengthen a portfolio.</p>

                <form wire:submit="save" class="mt-7 space-y-5">
                    <div class="grid gap-5 sm:grid-cols-2">
                        <label class="block">
                            <span class="text-sm font-bold">Role / title</span>
                            <input wire:model="title" type="text" class="mt-2 w-full rounded-2xl border-slate-300 bg-white px-4 py-3 text-slate-950 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100">
                        ...
                        <label class="block">
                            ...
                            <input wire:model="company" type="text" class="mt-2 w-full rounded-2xl border-slate-300 bg-white px-4 py-3 text-slate-950 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100">
                    ...
                    <div class="grid gap-5 sm:grid-cols-2">
                        <label class="block">
                            ...
                            <input wire:model="employmentType" type="text" placeholder="Full-time, Contract, Internship…" class="mt-2 w-full rounded-2xl border-slate-300 bg-white px-4 py-3 text-slate-950 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100">
                        </label>
                        <label class="block">
                            <span class="text-sm font-bold">Location</span>
                            <input wire:model="location" type="text" placeholder="Jakarta / Remote" class="mt-2 w-full rounded-2xl border-slate-300 bg-white px-4 py-3 text-slate-950 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100">
                        </label>
                    </div>
                    ...
                    <div class="grid gap-5 sm:grid-cols-2">
                        <label class="block">
                            ...
                            <input wire:model="startDate" type="date" class="mt-2 w-full rounded-2xl border-slate-300 bg-white px-4 py-3 text-slate-950 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100">
                        ...
                        <label class="block">
                            ...
                            <input wire:model="endDate" type="date" @disabled($isCurrent) class="mt-2 w-full rounded-2xl border-slate-300 bg-white px-4 py-3 text-slate-950 disabled:bg-slate-100 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:disabled:bg-slate-800">
                    ...
                    <label class="block">
                        ...
                        <textarea wire:model="description" rows="5" class="mt-2 w-full rounded-2xl border-slate-300 bg-white px-4 py-3 text-slate-950 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100" placeholder="Explain the scope of responsibility and the problems you owned."></textarea>
                    ...
                    <label class="block">
                        ...
                        <textarea wire:model="achievementsInput" rows="6" class="mt-2 w-full rounded-2xl border-slate-300 bg-white px-4 py-3 text-slate-950 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100" placeholder="Reduced deployment errors by…&#10;Led delivery of…&#10;Automated…"></textarea>
                    </label>

                    <label class="block">
                        <span class="text-sm font-bold">Technologies — comma separated</span>
                        <input wire:model="technologiesInput" type="text" placeholder="Laravel, Livewire, PostgreSQL, Vercel" class="mt-2 w-full rounded-2xl border-slate-300 bg-white px-4 py-3 text-slate-950 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100">
                    </label>
                ...
                <div class="mt-7 space-y-4">
                    @forelse ($careers as $career)
                        <article class="rounded-3xl border border-slate-200 p-6">
                            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                                <div>
                                    <div class="flex flex-wrap items-center gap-2">
                                        ...
                                        <span class="rounded-full px-2.5 py-1 text-[11px] font-black uppercase {{ $career->is_public ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-500 dark:text-slate-400' }}">{{ $career->is_public ? 'Public' : 'Private' }}</span>
                                    </div>
                                    <h3 class="mt-3 text-xl font-black">{{ $career->title }}</h3>
                                    <p class="mt-1 text-sm font-semibold text-slate-600 dark:text-slate-300">{{ $career->company }}@if($career->employment_type) · {{ $career->employment_type }}@endif</p>
                                </div>
                            ...
                            <p class="mt-4 text-sm leading-6 text-slate-600 dark:text-slate-300">{{ $career->description }}</p>
                    ...
                    @empty
                        <p class="rounded-3xl border border-dashed border-slate-300 p-8 text-slate-600 dark:text-slate-300">No career entries yet.</p>
                    @endforelse
                </div>
            </section>
        </div>
